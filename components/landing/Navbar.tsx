    ...
    <header className="w-full bg-white border-b border-gray-100 sticky top-0 z-40 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Profile Button — shows saved name if available */}
          <button
            onClick={onProfileClick}
            title={savedProfile ? `Profile: ${savedProfile.name}` : 'Set up your profile'}
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-lg transition-colors border cursor-pointer ${
              savedProfile
          ...
          >
            ...
            <span className="hidden sm:inline max-w-[100px] truncate">
              {savedProfile ? savedProfile.name : 'My Profile'}
            </span>
          </button>
